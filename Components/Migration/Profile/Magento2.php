...
class Magento2 extends Profile
{
    ...
    {
        return '/pub/media/catalog/product';
    }
    ...
    public function getPropertyOptionSelect()
    {
        return "
			...
			FROM catalog_eav_attribute eav_settings
    ...
    public function getProductPropertiesSelect($id)
    {
        return "
            ...
            WHERE p.type_id =  'catalog_product'
            AND p.entity_id = {$id}
		";
    }
    ...
    public function getProductsWithPropertiesSelect()
    {
        return "
			...
			FROM catalog_product_entity_int entity_int
    ...
    public function getAdditionalProductSelect($productId)
    {
        return "
            ...
            WHERE (p.type_id = 'simple' OR p.type_id = 'configurable')
            AND p.entity_id = {$productId}
            AND (parent.type_id = 'configurable' OR parent.type_id IS NULL)
            GROUP BY productID
        ";
    }
        ...
        return "
			...
			SELECT
              ...
              ''))		as billing_salutation,
				...
				TRIM(CONCAT(customer_address.firstname, ' ', IFNULL(customer_address.middlename, '')))
														as billing_firstname,
				customer_address.lastname 				as billing_lastname,
				customer_address.street					as billing_street,
				customer_address.city					as billing_city,
				customer_address.country_id				as billing_country,
				customer_address.postcode				as billing_zipcode,
        ...
        foreach ($attributes as $attribute) {
            if (empty($attribute_fields[$attribute])) {
                ...
                if ($attribute_fields[$attribute]['type'] === 'static') {
                    ...
                    $join_fields .= "
						LEFT JOIN {$type}_entity_{$attribute_fields[$attribute]['type']} $attribute
